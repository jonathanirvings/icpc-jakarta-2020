...
def main(argv):
  ...
  for _ in range(Q):
    ...
    if ops == 1:
      X = vr.ReadInt(1, N)
      assert vr.ReadChar() == ' ', 'Input is not space separated'
      Y = vr.ReadInt(1, 100000)
      assert vr.ReadChar() == ' ', 'Input is not space separated'
      K = vr.ReadInt(1, 100000)
    elif ops == 2:
      X = vr.ReadInt(1, N)
      has_second_type_query = True
    vr.ReadEndl()

  assert has_second_type_query, 'No query of the second type'

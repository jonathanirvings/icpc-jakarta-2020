...
def main(argv):
  ...
  for _ in range(N):
    ...
    S = vr.ReadString()
    assert S in ['RED', 'BLACK'], 'S is neither RED nor BLACK'
    vr.ReadEndl()
    
    numbers.append(A)

  B = vr.ReadIntList(M, 0, 10**9)

  numbers += B
  assert len(numbers) == len(set(numbers)), 'Found duplicates in A union B'
